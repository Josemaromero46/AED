...
class LinkedList(object):      # A linked list of data elements
   ...
   def setFirst(self, link):   # Change the first link to a new Link
      ...
      ###############
      if  self.__last is None: # Primer caso
         self.__last = link

      self.__last.setNext(self.__first) #haciendo circular
      ###############

   def setLast(self, link): 
      pass     #Hay que implementarlo, similar a setFirst
   ...
   def __len__(self):          # Get length of list
      ...
      link = self.getFirst()   # Start with first link
      # while link is not None:  # Keep going until no more links
      while link is not self.getLast():  # Keep going until no more links
   ...
   def insert(self, datum, last=False):    # Insert a new datum at start of list

...
class HashTable:
    def __init__(self, size=128):
        ...
        self.keys = [None] * size  # Lista de claves
        self.values = [None] * size  # Lista de valores
        self.used_cells = 0  # Número de celdas utilizadas

    def __len__(self):
        # Devuelve el número de celdas utilizadas
        return self.used_cells
    
    def __contains__(self, key):
        # Comprueba si una clave está en la tabla
        return self.get(key) is not None
    
    def hash_function(self, key):
        # Función de hash para calcular el índice en la tabla
        return hash(key) % self.size
    
    def rehash(self, old_hash):
        # Resuelve colisiones al calcular un nuevo índice
    ...
    def __setitem__(self, key, value):
        # Asignar un valor a una clave en la tabla
        index = self.hash_function(key)
        while self.keys[index] is not None:
            if self.keys[index] == key:
    ...
    # PARTE EDITADA 
    # mantiene el tamaño de la tabla como una potencia de 2 cuando se duplique      
    def __growTable(self):
        # Duplicar el tamaño de la tabla cuando el factor de carga se excede
        old_keys = self.keys
...
def test_hash_table():
    ...
    for lf in load_factors:
        for ps in probe_sequences:
            print(f"\nProbando load_factor={lf} y probe_sequence={ps}...")
